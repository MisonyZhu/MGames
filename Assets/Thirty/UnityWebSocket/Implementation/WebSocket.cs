...
namespace UnityWebSocket
{
    public class WebSocket : IWebSocket
    {
        ...
        public WebSocket(string address,bool isWorker=false)
        {
        ...
        public void ConnectAsync()
        {
            ...
            int code = WebSocketManager.WebSocketConnect(instanceId,IsWorker);
        ...
        public void SendAsync(byte[] data, int Position)
        {
            Log($"Send, type: {Opcode.Binary}, size: {Position}");
            int code = WebSocketManager.WebSocketSend(instanceId, data, Position);
            if (code < 0) HandleOnError(GetErrorMessageFromCode(code));
        }
