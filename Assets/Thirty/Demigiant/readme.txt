...
Dotween  日常使用api
1）Position 第一个参数  变换后的坐标   第二个参数  持续时间
	1）改变世界坐标
	        移动方法，第一个参数是要移动到的目标点，不是移动这个向量的距离
	        transform:DOMove(Vector3(1, 1, 1), 2)
	        只控制x轴上的移动，其他两个方向同理
	        transform:DOMoveX(1, 2)
	 2）改变局部坐标
	        transform:DOLocalMove(Vector3(1, 1, 1), 2)
	        transform:DOLocalMoveX(1, 2)
2）Rotation  
	1）世界旋转
        旋转到给定的值，改变的是欧拉角
        transform:DORotate(Vector3(0, 90, 0), 2)
        旋转到给定的值，改变的是四元数
        transform:DORotateQuaternion(Quaternion(0.1, 0.1, 0.1, 0.1), 2)

     2）局部旋转
     旋转到给定的值，改变的是欧拉角
        transform:DOLocalRotate(Vector3(0, 90, 0), 2)
        旋转到给定的值，改变的是四元数
        transform:DOLocalRotateQuaternion(Quaternion(0.1, 0.1, 0.1, 0.1), 2)
        
        在给定时间内，平滑的让自身的z轴正方向指向目标点
        transform:DOLookAt(Vector3(0, 0, 0), 2)
3) Scale
		transform:DOScale(Vector3(2, 2, 2), 2)
		其他两个轴向同理
...
4) Punch  同一方向上的晃动   
        ...
        transform:DOPunchPosition(Vector3(0, 10, 0), 2, 2, 0.1)
        transform:DOPunchRotation(Vector3(0, 90, 0), 2, 2, 0.1)
        transform:DOPunchScale(Vector3(2, 2, 2), 2, 2, 0.1)
5）Shake 随机晃动
...
6）Blend 
        ...
        1）允许多个同时执行
        例如：
        transform:DOBlendableMoveBy(Vector3(1, 1, 1), 1)
        transform:DOBlendableMoveBy(Vector3(-1, 0, 0), 1)
...
(2) Camera拓展方法
        ...
        7）按照屏幕像素计算的显示范围
        camera:DOPixelRect(CS.UnityEngine.Rect(0, 0, 600, 500), 2)
		8）按照屏幕百分比计算的显示范围
        camera:DORect(CS.UnityEngine.Rect(0.5, 0.5, 0.5, 0.5), 2)
        9）相机震动
        ...
        随机性：改变震动方向的随机值（大小：0~180）
        淡出：就是运动最后是否缓慢移动回到原本位置
        camera:DOShakePosition(1, 1, 5, 10, true)
(3) Material拓展方法  我没试过
		1）改变颜色
...
二、Dotween常用方法
    (1) Sequence
                local quence = CS.DG.Tweening.DOTween.Sequence()
      ...
      3）按时间点插入动画
        ...
        quence:Insert(0, transform:DORotate(Vector3(0, 90, 0), 1))
        
      4）加入当前动画
        Join会加入和让动画与当前正在执行的动画一起执行
        如下两行代码，DOMove会和DOScale一起执行
        quence:Append(transform:DOScale(Vector3(2, 2, 2), 2))
        quence:Join(transform:DOMove(Vector3.zero, 2))
        
      5）预添加动画
        预添加 会直接添加动画到Append的前面，也就是最开始的时候
        quence:Prepend(transform:DOScale(Vector3.one * 0.5, 1))
        这里需要特别说一下预添加的执行顺序问题
...
(2) Tweener的设置
      ...
      1）设置动画循环 
        ...
        para:SetLoops(-1, CS.DG.Tweening.LoopType.Yoyo)
...
(4) 回调函数
         1）动画完成回调
            transform:DOMove(Vector3.one, 2):OnComplete(function()end)
            
          2）动画被杀死时回调
            transform:DOMove(Vector3.one, 2):OnKill(function()end)
            
          3）动画播放时回调,暂停后重新播放也会调用
            transform:DOMove(Vector3.one, 3):OnPlay(function()end)
            
          4）动画暂停时回调
            transform:DOMove(Vector3.one, 2):OnPause(function()end)
          ...
          5）动画回退时回调
            ...
            transform:DOMove(Vector3.one, 2):OnRewind(function()end)
            
          6）只在第一次播放动画时调用，在play之前调用
            transform:DOMove(Vector3.one, 2):OnStart(function()end)
            
          7）完成单个循环周期时触发
            transform:DOMove(Vector3.one, 2):OnStepComplete(function()end)
          8）帧回调
            transform:DOMove(Vector3.one, 2):OnUpdate(function()end)
            
          9）在路径动画时，改变目标点时的回调，参数为当前目标点的下标
            transform:DOMove(Vector3.one, 2):OnWaypointChange(function(value)end)
(5) 动画控制方法
      1)播放
        transform:DOPlay()
...
(6) 获取数据方法
  ...
  二、实例方法
      ...
      13）返回循环次数，  无限循环为Infinity
        _tweener:Loops()
(7) 携程方法
     Coroutine.Start(LoginUI.xxx, LoginUI)
    {
        _tweener = transform.DOMove(Vector3.one, 2)
        
      1)等待动画执行完
      Coroutine.Yield(_tweener:WaitForCompletion())
        
      2）等待指定的循环次数
        参数为执行次数，等待传入的循环次数后，继续执行
        若是传入的次数大于动画的循环次数 则在动画结束时继续执行
         Coroutine.Yield(_tweener:WaitForElapsedLoops(2))
        
      3）等待动画被杀死
        Coroutine.Yield(_tweener:WaitForKill())
        
      4）等待动画执行指定时间
        参数为时间，动画执行传入的时间之后或动画执行完毕，继续执行
        Coroutine.Yield(_tweener:WaitForPosition(0.5))
      ...
      5）等待动画回退
        ...
        使用DOFlip翻转动画完成时
        使用DOPlayBackwards反向播放动画完成时
        Coroutine.Yield(_tweener:WaitForRewind())
        
      6）等待Start执行后继续执行
        Coroutine.Yield(_tweener:WaitForStart())
    }
